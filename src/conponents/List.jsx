...
export default function List() {
    ...
    useEffect(() => {
        const notActivatedList = list.filter((item) => !item.active);
        if (!notActivatedList.length) clearInterval(intervalId);
    }, [animals]);
        ...
        {animals.map((item, key) =>
            item.active ? (
                <tr className={'borderclass'} key={key}>
                    ...
                    <td className={'borderclass'}>{item.icon}</td>
                </tr>) :
                (<tr className={'borderclass'} key={key}>
                    <td className={'borderclass'}>{item.type}</td>
                    <td className={'borderclass'}>{item.icon}</td>
                </tr>)
        )}{" "}
        </tbody>
